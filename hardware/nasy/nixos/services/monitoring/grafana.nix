...
in
{
  services = {
    ...
    grafana-to-ntfy = {
      enable = config.services.ntfy-sh.enable;
      settings = rec {
        ntfyUrl = config.services.ntfy-sh.settings.base-url + "/grafana-to-ntfy";
        port = 6113;
        bauthUser = "grafana";
        bauthPass = config.age.secrets.ntfy-default-pass.path;
        ntfyBAuthUser = bauthUser;
        ntfyBAuthPass = bauthPass;
        markdown = true;
      };
    };
  };
